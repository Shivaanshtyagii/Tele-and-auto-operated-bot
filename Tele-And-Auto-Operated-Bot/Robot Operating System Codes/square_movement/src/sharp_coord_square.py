...
def distance(goal_pos):
    return math.sqrt(math.pow(goal_pos.x - curr_posx, 2) + math.pow(goal_pos.y - curr_posy, 2))

# def linear_velocity(goal_pos):
#     return 1.5 * distance(goal_pos)
...
def coordinate_square():
    ...
    while distance(goal_pos) >= 0.1 and not rospy.is_shutdown():
        print(distance(goal_pos), curr_posx, curr_posy)
        velocity.linear.x = 0.4
        velocity.angular.z = 0
        pub.publish(velocity)
    input('Enter the next x cp-')
    velocity.angular.z = 0
    velocity.linear.x = 0
    pub.publish(velocity)
    rospy.spin()
    ...
    #     # velocity.linear.x = 0
    #     velocity.angular.z = math.atan2(goal_pos.y - curr_posy, goal_pos.x - curr_posx)
    #     pub.publish(velocity)
    #     rospy.Rate(1).sleep()
    ...
    #         velocity.linear.x = linear_velocity(goal_pos)
    #         pub.publish(velocity)
